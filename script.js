...
// to know the start of the mouse when we clicking
...
function handleMouseMove(e) {
  if (isDrawing) {
    ...
    //to move the shape when we click on it and move the mouse
...
function handleMouseUp(e) {
  ...
  if (isDrawing && startX != e.offsetX && startY != e.offsetY) {
    ...
    // deselect the shape when we finish moving it
  }else if(isDrawing==false && shapeToDraw=="move"){
    selectedShape.color = 'black'
    selectedShape = null;
  }
...
function drawShape(beginX = startX, beginY = startY, currentX, currentY) {
  if (shapeToDraw === "rectangle") {
    ...
    ctx.arc(
      ...
      beginY,
      Math.sqrt(
        Math.pow(currentX - beginX, 2) + Math.pow(currentY - beginY, 2)
      ),
      0,
      2 * Math.PI
    );
